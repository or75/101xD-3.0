...
void cNoSpread::HUD_PostRunCmd(struct local_state_s *from, struct local_state_s *to, struct usercmd_s *cmd, int runfuncs, double time, unsigned int random_seed)
{
	...
	if (runfuncs)
	{
		...
		if (cmd->buttons & IN_ATTACK /*&& g_Local.iWeaponID!=WEAPONLIST_C4&&g_Local.iWeaponID!=WEAPONLIST_HEGRENADE&&g_Local.iWeaponID!=WEAPONLIST_KNIFE&&g_Local.iWeaponID!=WEAPONLIST_FLASHBANG&&g_Local.iWeaponID!=WEAPONLIST_SMOKEGRENADE*/) // catch case when pistol and IN_ATTACK is always on and not firing
		{
			PrimaryAttack();
		}
